...
class CustomCompose:
    ...
    def __init__(self, transforms, ft_begin_epoch):
        assert isinstance(transforms, collections.abc.Sequence)
        self.ft_begin_epoch = ft_begin_epoch
        self.transforms = []
    ...
    def __call__(self, data):
        ...
        for t in self.transforms:
            if 'ObjectSample'  in t.__class__.__name__ and self.ft_begin_epoch is not None:
                if self.__getattribute__('epoch') >= self.ft_begin_epoch:
                    continue
            data = t(data)
            if data is None:
                return None
