...
class CustomCollect3D(Collect3D):
    def __init__(
        ...
        super(CustomCollect3D, self).__init__(keys, meta_keys)
        self.debug = debug
    ...
    def __call__(self, results):
        results = super(CustomCollect3D, self).__call__(results)
        if self.debug:
            vis_img = True
            vis_2d = False
            vis_3d = False
            ...
            for img_id, (img, img_meta) in enumerate(zip(results['img'].data, img_metas)):
                ...
                if vis_3d:
                    ...
                    img_3d = draw_lidar_bbox3d_on_img(None, img_3d, lidar2img, None)
                ...
                if vis_bbox:
                    ...
                    intrinsics, extrinsics = self.get_box_params(
                        [bboxes_2d.int().float()], [img_meta['cam_intrinsic']], [img_meta['lidar2cam']], roi_size)
                    ...
                    for i in range(len(bboxes_2d)):
                        ...
                        lidar2img = (intrins @ extrins).numpy()
